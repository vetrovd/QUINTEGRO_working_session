...
import { BOOKING_ID, CTX, NOW, TODAY, booking, confirmed, lastRejection, readyToStart } from "./fixtures";
...
const checkIn = (id: string) => ({ type: "VisitCheckedIn", visitId: id }) as const;
...
describe("отметка прихода", () => {
  ...
  it("переводит визит в «пришёл» и фиксирует время", () => {
    ...
    expect(state.visits[todayMorning]).toMatchObject({ status: "checkedIn", checkedInAt: NOW });
  });
  ...
  it("второй приход не сдвигает время начала опеки", () => {
    ...
    const second = reduce(first, checkIn(visitId(BOOKING_ID, TODAY, "evening")), {
